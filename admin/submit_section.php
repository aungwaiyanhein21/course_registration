<?php
    ...
    $query = "SELECT MAX(letter) AS ml
                    FROM Section
                    WHERE course_id = $s->course_id
                    ";
    $latest_section = $file_db->query($query);
   // $res = $latest_section->fetch(PDO::FETCH_ASSOC);
    
    $s->letter = "A";

    foreach($latest_section as $ls){
            //echo("Letter: ".$ls['ml']);
            if($ls['ml']){
                $s->letter = ++$ls['ml'];
            }
